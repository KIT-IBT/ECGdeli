% -------------------------------------------------------
%
%    Sync_Channels  - Synchronize FPTs comming from different channels of the same signal
%
%    Ver. 1.0.0
%
...
% ------------------------------------------------------
%
% [FPT_Synced,FPT_Cell_Synced]=Sync_Channels(FPT_Cell,samplerate)
...
%
% Example Usage:
%       [FPT_Synced,FPT_Cell_Synced]=Sync_Channels(FPT_Cell,250)
%
% Revision history:
%
%
...
for n=1:N_Channels
    ...
    class_mat=comp_mat; %gives class of the QRS complex in each FPT table. Every colum represents a channel. 
    ...
    for i=1:size(FPT_Cell{n},1)
        ...
        for j=1:N_Channels
            if isempty(FPT_Cell{j})
                ...
                if Timedif<=Time_Limit %check if minimal time difference is smaller than Time_Limit
                    ...
                    class_mat(i,j)=FPT_Cell{j}(pos,13); %class of the ith QRS complex of the nth FPT in the jth FPT
                end
            end
        end
        ...
        if V>=Voting_Threshhold %QRS complex is accepted if the relative number of votes is bigger than Voting_Threshold
            ...
            Class=Sync_Class(class_mat(i,comp_mat(i,:)==1)); %sync class for the accepted QRS
            
            M=cat(1,M,[PonPOS,PpeakPOS,PoffPOS,QRSonPOS,QPOS,RPOS,SPOS,QRSoffPOS,JPOS,TonPOS,TpeakPOS,ToffPOS,Class]); %aggregate new fiducial points to the existing ones
            for l=1:N_Channels
                if comp_mat(i,l)
                    FPT_Cell_Synced{l}=cat(1,FPT_Cell_Synced{l},[FPT_Cell{l}(pos_mat(i,l),1:12),Class]);
                else
...
[~,ind]=sort(M(:,3)); %Sort the position of the QRS complexes in the signal
FPT_Synced=M; %Relocate positions of all ECG waves
for i=1:N_Channels
